...
def generate_response(model_name: str, 
    ...
    # print(device_map)
    if model_name == 'InternVL2-Llama3-76B-8bit':
        device_map = split_model(model_name=model_name.strip('-8bit'))
        model_path = model_path.strip('-8bit')
        ...
        device_map = split_model(model_name=model_name)
        model = AutoModel.from_pretrained(
            model_path,
            torch_dtype=torch.bfloat16,
            low_cpu_mem_usage=True,
    ...
    generation_config = dict(max_new_tokens=MAX_TOKENS, 
                             do_sample=False)
    ...
    for query in tqdm(queries):
        ...
        pixel_values, num_patches_list = load_video(video_path, num_segments=total_frames, max_num=1)
        ...
        prompt = video_prefix + question_prompt
